...
function simone_paging_nav() {
	...
	$links = paginate_links( array(
		...
		'prev_text' => __( '← Previous', 'simone' ),
		'next_text' => __( 'Next →', 'simone' ),
                'type'      => 'list',
	) );
...
function simone_posted_on() {
	...
	printf( __( '<span class="byline">Written by %2$s</span><span class="posted-on">%1$s</span>', 'simone' ),
...
function simone_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'all_the_cool_cats' ) ) ) {
		...
		$all_the_cool_cats = get_categories( array(
			...
			// We only need to know if the re is more than one category.
			'number'     => 2,
		) );
